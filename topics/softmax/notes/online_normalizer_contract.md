...
### 首轮探索记录与审计修正（2026-09-05，探索性，未确认）

原 scratch 摘要保留在 Git revision `bf632c3`。其中报告小块数误差约
$3\times10^{-8}\sim9\times10^{-8}$，以及较大块数下 chain 与 balanced 的差距增大。
随机扫描的逐族输入、种子、完整调度和汇总脚本未随该提交保存，不能据此重放各格均值，
也不能将其称为已冻结的设计约束。以下可重现算例用于审查解读，不替代随机扫描的确认。

**FP32 输出量化不是参照的测量分辨率。** 正规区内相对 ULP 约在
$[2^{-24},2^{-23}]$；具体值依赖根的位置，不恒等于 $6\times10^{-8}$。
在根 32 处，ULP 为 $2^{-18}$、相对 ULP 为 $2^{-23}\approx1.192\times10^{-7}$。
正确舍入误差至多半 ULP，但可以为零；这不是所有输入共有的正误差地板。
即使两个输出只差一个 FP32 格点，也可以对高精度共同参照明确判定哪个更准。

例如 `max_at_position(32, 32, Fraction(20), 31)`、默认 correctly-rounded exp、非 FMA：
chain 输出 $32+2^{-18}$，balanced 输出 32，相对误差分别约为
$5.5313523721\times10^{-8}$ 与 $6.3895758213\times10^{-8}$。
参照分母区间宽约 $3.70\times10^{-141}$，配对误差差区间严格为负，
`error_order` 返回 -1；chain 在此还恰好得到正确舍入的根。
因此“32 块分不开”“必须从 512 块起”均不成立。增大块数可以作为进一步探索累积误差的轴，
小块数应保留为对照；是否有跨输入族的稳定效应需要另做抽样和统计确认。

**误差比值不是自动成立的下界。** $A_{\rm chain}/A_{\rm balanced}$ 是两个实际误差的比，
balanced 接近正确舍入不构成观测被截断的证据，也推不出关于另一种“真实差距”的不等式。
如需报告比值，须注明是均值之比还是逐族比值的均值；分母接近零时尤其不稳定。
本阶段仍以同族配对差为 primary。单个输入的数值参照区间与总体效应的统计置信区间必须分开；
四个族的描述性摘要不能替代后者。

**吸收次数不能直接排序最终误差，有一个确定性反例。** 使用同样的 32 个等质量块，
仅把 max 位置设为 0，即 `max_at_position(32, 32, Fraction(20), 0)`，默认 exp、非 FMA。
chain、balanced、`warp_shfl_down(32)`、`split_k(32, 4)` 的吸收次数是 31 / 5 / 5 / 10，
但输出都恰好为 32，因而对同一个实数参照的相对误差完全相同，约为 $6.3895758213\times10^{-8}$。
平衡树让 max 依次接收含 1、2、4、8、16 个低 max 块的分组；各组重标定后的贡献都不足半 ULP，
故同样的质量可以由不同数量的事件丢失。这否定了“事件越多，最终误差必然越大／按比例增大”，
不证明计数在任何分布上都没有统计信息，也不是所有四种 schedule 普遍等价的结论。
上述两个算例已加入 `test_online_pilot.py`，输入和设置均可复现。

后续扫描须记录输入 FP32 位模式、块大小、种子、实际 schedule、FMA/exp 设置、逐族结果与
汇总规则。`warp_shfl_down` 只用于支持的 lane 数；不支持的宽度记为不适用，不能把 balanced
回填后继续作为独立 shfl 结果。先补运行前预测，再固定下一轮范围，不能用首轮观察回填预测。
