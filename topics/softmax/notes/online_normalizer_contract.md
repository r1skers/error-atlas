...
### 首轮探索跑出的设计约束（2026-09-05，scratch，非证据）

用 pilot 试跑后发现两件事，都影响正式一轮怎么设计。

**块数必须够大，否则指标没有动态范围。** 32 块时全部 schedule 的相对误差都落在
$3\times10^{-8}\sim9\times10^{-8}$，而根附近一个 ULP 的相对大小就是 $6\times10^{-8}$——
误差被量化到少数几个可能值上，schedule 之间根本分不开。块数加到 512 以上才拉开：
chain 的误差随块数增长，balanced 基本不动。因此正式一轮的块数应从 512 起，
且要记住 balanced 那一侧仍在地板上，比值只能当差距的**下界**读。

**吸收次数不能当代理指标，这一条现在有实证。** 取 32 块、max 排在位置 0，
四个 schedule 的吸收次数分别是 31 / 5 / 5 / 10，而最终相对误差**完全相同**。
原因是平衡树让 max 依次吞掉大小 2、4、8、16 的分组，每组单独都不到半个 ULP，
于是照样全丢。这正是 §5 "吸收是连续的，不是二值的"那条的实例：
计数差 6 倍而损失质量相同，用计数会得出完全错误的排序。
