...
误差来源（本模块记账的是哪一种、不记哪一种）：
  记：每个算子的一次舍入残差（加、减、乘各一次；FMA 对 a*b+c 整体只舍一次）。
    这**包括** Delta-m 那次减法的残差 eta —— ``fp32_sub`` 照常算出并返回它。
  不记：输入的表示误差（叶子必须已是 stored FP32）；exp 实现自身的误差，
    因为 exp 不是本模块的算子。
  分层不要混：eta 在**算术层**由本模块算出并返回，但在**归因层**不出现在
    frozen-weight 恒等式的求和里，因为它已被吸收进冻结的 w_hat（合同 §4）。
    "本模块不算它"和"恒等式里没有它"是两句不同的话，只有后者成立。
伪代码：
  round_to_fp32(v):
      negative <- v < 0;  m <- |v|
