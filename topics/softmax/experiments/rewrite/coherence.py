...
"C 主导"在这里的可操作定义是什么（用什么统计量、跨什么比较）：固定一组输入，
  换多棵树，看跨树的 std(C)/std(A)。A 在树间几乎不变，好坏几乎全来自 C。
伪代码：A = Σδ²；用前缀和一趟算 C = Σ_v 2·δ_v·(前缀和)；再用 E²-A 核对。

Prediction record（跑测试后回填）
--------------------------------
Direction（在 wide-range 输入上，跨树看 C 的离散程度比 A 大还是小）：C 的离散明显更大。
Scale（大概几倍）：宽度 256 上 std(C)/std(A) 实测 2.5 到 3.6 倍。
Boundary（哪种输入或树会让 A 和 C 差不多大）：符号不相关时，或某个 δ 独大时，C 相对 A 变小
Failure signature（如果 C 的两种算法不一致，最可能是哪里错）：前缀和更新顺序写反
  （先并入 d 再配对），会多算一个自配对项，两路核对立刻不等。
