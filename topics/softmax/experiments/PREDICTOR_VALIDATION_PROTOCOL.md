...
The completed `wide_range_offline_tree_reuse_v1` stage tested the corresponding cost-first
deployment contract.  It froze one 64-tree catalog per width, selected one reusable tree from 32
fresh calibration inputs using the unchanged K8/B3 score, and evaluated it on 64 fresh confirmation
inputs.  The reusable tree beat random fixed-catalog expected regret, but did not beat balanced
FP32; even the calibration-label static policy failed that engineering gate.  FP64-then-FP32 was
