...
进入以研究结论为目标的确认实验/统计工具开发之前，与预测记录一起先写一页内的筛查：

- 最接近的已有结果及可核查的一手来源是什么？它的假设覆盖当前输入、算术和指标吗？
- 当前工作属于学习复现、已知机制的边界检查，还是有明确差异的新假说？
- 哪个最小基线/消融能区分已有解释与新假说？误差界、拟合曲线和机制证据须分开。
- 若目标是工程改进，最终使用者关心哪个输出、dtype、容差与成本？不能用中间量替代。
- 这一步的预算、结束条件和继续/归档规则是什么？未定义用途及容差时，只做有限诊断，
  不自动扩充确认工具或优化投入。

学习练习可按学习目标继续，不需要人为包装成新颖研究；用户显式指定模式仍优先。
Softmax 实例见[已知性筛查 v1](../topics/softmax/notes/online_prior_work_screening_v1.md)。
